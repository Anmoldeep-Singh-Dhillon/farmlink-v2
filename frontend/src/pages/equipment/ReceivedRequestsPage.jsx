...
import { getReceivedRequests, acceptRentalRequest, rejectRentalRequest } from '../../api/equipment'
...
export default function ReceivedRequestsPage() {
  ...
  const acceptMutation = useMutation({
    mutationFn: acceptRentalRequest,
    onSuccess: () => {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Rental Requests Received</h1>

      {requests.length === 0 && (
        <div className="text-center py-12 text-gray-500">No requests received yet</div>
      )}
            ...
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  Request #{req.id} • {new Date(req.requestedAt).toLocaleDateString()}
                </p>
              </div>
